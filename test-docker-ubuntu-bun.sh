...
# Ubuntu + Bun Docker Memory Leak Test
# Tests Bun runtime in Ubuntu 22.04 container to isolate containerization effects

set -e

echo "🔬 Ubuntu + Bun Docker Memory Leak Test"
echo "======================================="
...
# Build Docker image
echo -e "${BLUE}🏗️  Building Ubuntu + Bun Docker image...${NC}"
docker build -f Dockerfile.ubuntu-bun -t ubuntu-bun-memory-leak-repro .

# Start container in background
echo -e "${BLUE}🚀 Starting Ubuntu + Bun Docker container...${NC}"
CONTAINER_ID=$(docker run -d -p 3000:3000 --name ubuntu-bun-memory-leak-test ubuntu-bun-memory-leak-repro)
...
echo -e "${GREEN}✅ Ubuntu + Bun Docker container is running and server is ready${NC}"

# Show container info
echo -e "${BLUE}📦 Container Information:${NC}"
echo "Container ID: $CONTAINER_ID"
echo "Base Image: Ubuntu 22.04"
echo "Runtime: Bun (installed in Ubuntu container)"
echo "Image: ubuntu-bun-memory-leak-repro"
echo ""
...
if $JQ_AVAILABLE; then
    ...
    if [ $HEADERS_DIFF -gt 500 ] || [ $RESPONSES_DIFF -gt 500 ] || [ $SOCKETS_DIFF -gt 100 ]; then
        ...
        echo -e "${RED}🚨 CRITICAL MEMORY LEAK DETECTED IN UBUNTU + BUN!${NC}"
        ...
        echo -e "${YELLOW}⚠️  MODERATE MEMORY LEAK DETECTED IN UBUNTU + BUN${NC}"
        echo "Some objects are not being garbage collected properly"
    else
        LEAK_SEVERITY="none"
        echo -e "${GREEN}✅ No significant memory leak detected in Ubuntu + Bun${NC}"
...
echo -e "${GREEN}✅ Ubuntu + Bun test completed!${NC}"
echo ""
echo -e "${BLUE}📋 Ubuntu + Bun Test Summary:${NC}"
echo "Duration: $DURATION seconds"
echo "Base Container: Ubuntu 22.04"
echo "Runtime: Bun (installed in Ubuntu container)"
echo "Container: ubuntu-bun-memory-leak-repro"
if $JQ_AVAILABLE; then
